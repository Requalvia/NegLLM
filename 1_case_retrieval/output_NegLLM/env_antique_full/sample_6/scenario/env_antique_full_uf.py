...
def uf_a(offer: dict):
    ...
    try:
        ...
        # Jade
        Jade_offer = offer["Jade"]
        cost_price_Jade = 400
        profit_Jade = Jade_offer - cost_price_Jade
        penalty_factor = y if profit_Jade < 0 else 1
        weighted_profit_Jade = profit_Jade * penalty_factor
        reward += weighted_profit_Jade
        explanation_parts.append(
            f"Jade: Profit {profit_Jade} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_profit_Jade:.1f} (Bought for {cost_price_Jade}, sold for {Jade_offer})"
        )
...
def uf_b(offer: dict):
    ...
    try:
        ...
        # Jade
        Jade_offer = offer["Jade"]
        market_price_Jade = 600
        gain_Jade = market_price_Jade - Jade_offer
        penalty_factor = y if gain_Jade < 0 else 1
        weighted_gain_Jade = gain_Jade* penalty_factor
        reward += weighted_gain_Jade
        explanation_parts.append(
            f"Jade: Gain {gain_Jade} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_gain_Jade:.1f} (Bought for {Jade_offer}, market {market_price_Jade})"
        )
