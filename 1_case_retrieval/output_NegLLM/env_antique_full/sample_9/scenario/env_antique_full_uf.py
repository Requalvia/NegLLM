...
def uf_a(offer: dict):
    ...
    try:
        ...
        # Vase the antique 2
        Vase_offer = offer["Vase"]
        cost_price_Vase = 450
        profit_Vase = Vase_offer - cost_price_Vase
        penalty_factor = y if profit_Vase < 0 else 1
        weighted_profit_Vase = profit_Vase * penalty_factor
        reward += weighted_profit_Vase
        explanation_parts.append(
            f"Vase: Profit {profit_Vase} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_profit_Vase:.1f} (Bought for {cost_price_Vase}, sold for {Vase_offer})"
        )
...
def uf_b(offer: dict):
    ...
    try:
        ...
        # Vase
        Vase_offer = offer["Vase"]
        market_price_Vase = 650
        gain_Vase = market_price_Vase - Vase_offer
        penalty_factor = y if gain_Vase < 0 else 1
        weighted_gain_Vase = gain_Vase * penalty_factor
        reward += weighted_gain_Vase
        explanation_parts.append(
            f"Vase: Gain {gain_Vase} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_gain_Vase:.1f} (Bought for {Vase_offer}, market {market_price_Vase})"
        )
