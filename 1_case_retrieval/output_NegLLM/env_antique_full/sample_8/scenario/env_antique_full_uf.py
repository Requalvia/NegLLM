...
def uf_a(offer: dict):
    ...
    - Weighted profit:
        Sculpture profit * (1 + x)
        Artifacts profit * (1 - x)
    ...
    try:
        ...
        # Sculpture
        Sculpture_offer = offer["Sculpture"]
        cost_price_Sculpture = 400
        profit_Sculpture = Sculpture_offer - cost_price_Sculpture
        penalty_factor = y if profit_Sculpture < 0 else 1
        weighted_profit_Sculpture = profit_Sculpture * penalty_factor
        reward += weighted_profit_Sculpture
        explanation_parts.append(
            f"Sculpture: Profit {profit_Sculpture} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_profit_Sculpture:.1f} (Bought for {cost_price_Sculpture}, sold for {Sculpture_offer})"
        )

        # Artifacts the antique 2
        Artifacts_offer = offer["Artifacts"]
        cost_price_Artifacts = 700
        profit_Artifacts = Artifacts_offer - cost_price_Artifacts
        penalty_factor = y if profit_Artifacts < 0 else 1
        weighted_profit_Artifacts = profit_Artifacts * penalty_factor
        reward += weighted_profit_Artifacts
        explanation_parts.append(
            f"Artifacts: Profit {profit_Artifacts} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_profit_Artifacts:.1f} (Bought for {cost_price_Artifacts}, sold for {Artifacts_offer})"
        )
...
def uf_b(offer: dict):
    ...
    try:
        ...
        # Sculpture
        Sculpture_offer = offer["Sculpture"]
        market_price_Sculpture = 600
        gain_Sculpture = market_price_Sculpture - Sculpture_offer
        penalty_factor = y if gain_Sculpture < 0 else 1
        weighted_gain_Sculpture = gain_Sculpture* penalty_factor
        reward += weighted_gain_Sculpture
        explanation_parts.append(
            f"Sculpture: Gain {gain_Sculpture} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_gain_Sculpture:.1f} (Bought for {Sculpture_offer}, market {market_price_Sculpture})"
        )

        # Artifacts
        Artifacts_offer = offer["Artifacts"]
        market_price_Artifacts = 900
        gain_Artifacts = market_price_Artifacts - Artifacts_offer
        penalty_factor = y if gain_Artifacts < 0 else 1
        weighted_gain_Artifacts = gain_Artifacts * penalty_factor
        reward += weighted_gain_Artifacts
        explanation_parts.append(
            f"Artifacts: Gain {gain_Artifacts} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_gain_Artifacts:.1f} (Bought for {Artifacts_offer}, market {market_price_Artifacts})"
        )
