...
def uf_a(offer: dict):
    ...
    try:
        ...
        # Ornament
        Ornament_offer = offer["Ornament"]
        cost_price_Ornament = 350
        profit_Ornament = Ornament_offer - cost_price_Ornament
        penalty_factor = y if profit_Ornament < 0 else 1
        weighted_profit_Ornament = profit_Ornament * penalty_factor
        reward += weighted_profit_Ornament
        explanation_parts.append(
            f"Ornament: Profit {profit_Ornament} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_profit_Ornament:.1f} (Bought for {cost_price_Ornament}, sold for {Ornament_offer})"
        )

        # Artifacts the antique 2
        Artifacts_offer = offer["Artifacts"]
        cost_price_Artifacts = 650
...
def uf_b(offer: dict):
    ...
    try:
        ...
        # Ornament
        Ornament_offer = offer["Ornament"]
        market_price_Ornament = 550
        gain_Ornament = market_price_Ornament - Ornament_offer
        penalty_factor = y if gain_Ornament < 0 else 1
        weighted_gain_Ornament = gain_Ornament* penalty_factor
        reward += weighted_gain_Ornament
        explanation_parts.append(
            f"Ornament: Gain {gain_Ornament} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_gain_Ornament:.1f} (Bought for {Ornament_offer}, market {market_price_Ornament})"
        )

        # Artifacts
        Artifacts_offer = offer["Artifacts"]
        market_price_Artifacts = 850
