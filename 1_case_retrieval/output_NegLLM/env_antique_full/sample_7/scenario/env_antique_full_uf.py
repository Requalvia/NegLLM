...
def uf_a(offer: dict):
    ...
    try:
        ...
        # Painting
        Painting_offer = offer["Painting"]
        cost_price_Painting = 100
        profit_Painting = Painting_offer - cost_price_Painting
        penalty_factor = y if profit_Painting < 0 else 1
        weighted_profit_Painting = profit_Painting * penalty_factor
        reward += weighted_profit_Painting
        explanation_parts.append(
            f"Painting: Profit {profit_Painting} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_profit_Painting:.1f} (Bought for {cost_price_Painting}, sold for {Painting_offer})"
        )
...
def uf_b(offer: dict):
    ...
    try:
        ...
        # Painting
        Painting_offer = offer["Painting"]
        market_price_Painting = 300
        gain_Painting = market_price_Painting - Painting_offer
        penalty_factor = y if gain_Painting < 0 else 1
        weighted_gain_Painting = gain_Painting* penalty_factor
        reward += weighted_gain_Painting
        explanation_parts.append(
            f"Painting: Gain {gain_Painting} "
            f"{' * y=' + str(y) + ' (loss penalty)' if penalty_factor > 1 else ''}"
            f" = {weighted_gain_Painting:.1f} (Bought for {Painting_offer}, market {market_price_Painting})"
        )
